...
uint32_t init_timer ( uint8_t timer_num, uint32_t TimerInterval )
{
  ...
  {
		//LPC_SC->PCONP|=(1<<1);
	LPC_TIM0->MR0 = TimerInterval;    //setto il registro MR0 a 1 s   //ogni secondo genero un iterruzione
...
//	 <i> 0 Feature disabled.
//   </e>
	LPC_TIM0->MCR = 3;  //genero un iterruzione su MR0 E MR1 resetto il registro MR1 setto il bit 0 , 3 ,4 ---> 1 + 8 + 16 = 25 per quello lo setto a 25
// </h>
//*** <<< end of configuration section >>>    ***

	NVIC_EnableIRQ(TIMER0_IRQn);
	NVIC_SetPriority(TIMER0_IRQn, 0);	/* more priority than timer2*/
	return (1);
  }
  else if ( timer_num == 1 )
  {
	...
	NVIC_SetPriority(TIMER1_IRQn, 0);	/* more priority than timer2*/
	return (1);
  }
  else if(timer_num == 2 ){
		LPC_TIM2->MR0 = TimerInterval/2;    //setto il registro MR0 a 1 s   //ogni secondo genero un iterruzione
		LPC_TIM2->MR1 = TimerInterval;			//setto il registro MR1 a 2 s	
		LPC_TIM2->MCR = 25;				/* Interrupt and Reset on MR1 */
		NVIC_EnableIRQ(TIMER2_IRQn);
		NVIC_SetPriority(TIMER2_IRQn, 1);	/* more priority than timer2*/
		return (1);
	}
  else if (timer_num==3){
		...
		NVIC_SetPriority(TIMER3_IRQn, 1);	/* more priority than timer2*/
		return (1);
